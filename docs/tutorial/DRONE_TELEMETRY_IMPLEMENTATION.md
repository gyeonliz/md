...
`HUD-01` 데이터 공급, `HUD-02` 표시, `TUT-01` Course, `TUT-02` Gate, `TUT-03` 기록과 `TUT-04B` 이전 평균·Best 결과를 구현했다. Lap Recorder는 이 문서의 10Hz Telemetry Event를 위치 표본 시계로 재사용한다. 현재는 실제 두 Lap 결과 확인이 남았고 다음 기능은 Flight 상태 또는 NavigationArrows Host/Wrapper다. 기록·비교 기준은 [`DRONE_TRAINING_RECORDING_IMPLEMENTATION.md`](DRONE_TRAINING_RECORDING_IMPLEMENTATION.md)를 따른다.
