...
Unreal 공유 기준선은 `main=origin/main=eeb4354`다. Smart Object Definition·Station Blueprint 6쌍과 NPC 역할 Blueprint 3종, Spawn Point Blueprint, 전용 Greybox 맵을 생성·검증했다. 이 준비 작업은 AI가 완성됐다는 뜻이 아니며, StateTree 순찰·아군 이동·사격은 별도 카드로 닫는다.
...
| 진행 정도 | 기존 TUT-04B는 실제 두 Lap 확인 대기. `AI-SO-00`, `AI-SO-01`, `AI-NPC-01` 기술 기반 완료 |
| 지금 작업 중 | `AI-NPC-01` 완료. 다음 카드 `AI-PATROL-01` 적 순찰 StateTree 준비 |
| 완료 근거 | Game/Editor Build 성공, NPC 전용 2/2, 전체 `Drone.` 20/20(19 성공+기존 PIE 경고 포함 성공 1), Blueprint 0/0/0, LFS fsck 통과 |
...
| 다음 행동 | `AI-PATROL-01`에서 Hostile이 EnemyPatrol Slot을 Claim·이동·대기·해제하는 최소 StateTree 구성 |
| 다음 기능 | `AI-PATROL-01 → AI-FRIEND-01 → AI-PER-01`; 이동·예약·감지가 안정된 뒤 Rifle·Shotgun 사격 |
...
| Git 처리 | AI-NPC-01 기능 `362edaa`, main Merge `eeb4354` Push 완료. AI-SO-01 Merge `89ca2bc`와 기존 사용자 `4f14d2f` Map 변경 보존 |
...
Unreal 프로젝트의 초기 Commit은 `91498b7`이고 현재 `main=origin/main=eeb4354`다. AI-NPC-01 기능 Commit은 `362edaa`, Merge는 `eeb4354`며 AI-SO-01 Merge `89ca2bc`, 기존 자산·TUT-04B Merge `55b3ffe`와 사용자 Battlefield Map 갱신 `4f14d2f`을 보존한다. 다른 PC Clone·LFS·UE 5.8.1 실행과 문서 Clone/Pull을 확인하기 전까지 PC 간 전체 공유 흐름은 완료로 닫지 않는다.
...
- TUT-04 실제 두 Lap 확인은 그대로 남긴다. 사용자 요청으로 `AI-NPC-01`까지 먼저 준비했고, 다음 구현은 `AI-PATROL-01`이다.
