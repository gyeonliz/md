...
19. [`docs/DRONE_TUTORIAL_STORY_PLAN.md`](docs/DRONE_TUTORIAL_STORY_PLAN.md): 확정 조작, Tutorial 코스·기록 UI, Story·NPC·Jamming·에셋 적용 계획
20. [`docs/DRONE_ASSET_INTAKE_2026-08-25.md`](docs/DRONE_ASSET_INTAKE_2026-08-25.md): 최초 D 드라이브 14팩 압축 감사, 다른 PC의 C 드라이브 재감사와 FPV·Loop 선별 이식 검증
21. [`docs/DRONE_REMAINING_ASSET_MIGRATION_2026-08-27.md`](docs/DRONE_REMAINING_ASSET_MIGRATION_2026-08-27.md): 남은 제공 자산 891개·OilRig 중앙 맵·TUT-04B 이식 및 검증
22. [`docs/DRONE_UNREAL_MCP.md`](docs/DRONE_UNREAL_MCP.md): UE 5.8 공식 Unreal MCP·Codex 연결, 선택 Toolset과 검증 기준
23. [`docs/DRONE_CHAOS_DATAFLOW_PLAN.md`](docs/DRONE_CHAOS_DATAFLOW_PLAN.md): UE 5.8 Dataflow 기반 부분 고정 그물·선택형 맵 파괴 설계와 검증 순서
24. [`docs/UNREAL_PROJECT_EXPERIENCE_DESCRIPTION.md`](docs/UNREAL_PROJECT_EXPERIENCE_DESCRIPTION.md): 지원서·이력서용 Unreal 프로젝트 경험 기술 예시와 사실 확인 경계
25. [`docs/STUDY_PLANS.md`](docs/STUDY_PLANS.md): 정보처리산업기사·C++ 코딩테스트 병행 계획
...
NPC·AI 다음 작업을 위해 Smart Objects와 Gameplay Interactions 모듈, Faction·Rifle·Shotgun Profile, Activity Tag, NPC Character/Controller/Spawn Point, Slot 예약 Component와 드론 Sight 감지 기반을 준비했다. Smart Object Definition·Station Blueprint 6쌍에 이어 Hostile Rifle·Shotgun, Friendly Base Blueprint 3종, Spawn Point BP와 `Lvl_NPCSmartObjectGreybox`을 생성했다. 맵에 NPC 4명과 Station 10개, 전용 Navigation Floor를 배치하고 역할 Profile·Possess·Activity Tag·NavMesh 투영을 NPC 전용 2/2와 전체 `Drone.` 20/20으로 검증했다. Manny/Unarmed 외형은 임시 Greybox이고 Interaction StateTree·실제 순찰·아군 이동·사격은 아직 미구현이며, 다음은 `AI-PATROL-01`이다.

2026-08-27 현재 Unreal 저장소 로컬 `main`과 `origin/main`은 AI-NPC-01 Merge Commit `eeb4354`로 일치한다. 기능 Commit은 `362edaa`, AI-SO-01 Merge는 `89ca2bc`다. 기존 남은 제공 자산·TUT-04B Commit은 `3fa4444`, Merge는 `55b3ffe`이며 신규 Unreal 패키지 892개·4.9GB를 Git LFS로 Push했다. 그 뒤 사용자 Battlefield Map Commit `4f14d2f`도 그대로 보존했다. 이식 자산은 후보 라이브러리이며 실제 외형 채택이나 완성된 AI·MG 기능을 뜻하지 않는다.
