...
| Unreal 프로젝트 저장소 | `main`=`origin/main`=`c3e6d38`; AI 기반 `489ced5` 병합·Push, 기존 사용자 Battlefield Map `4f14d2f` 보존 |
| 문서 작업 저장소 | 이 갱신을 기능 Branch와 `main`에 Commit·Push하고 로컬·원격 일치 상태로 마감 |
| Commit·Push 처리 | 기존 자산 LFS Push 뒤 AI 기반 기능 `489ced5`를 Merge Commit `c3e6d38`로 `origin/main`에 Push 완료 |
| 실행 상태 | Game/Editor Build 성공, 전체 `Drone.` 18/18(17 성공+기존 PIE 경고 포함 성공 1), Blueprint 0/0/0, LFS fsck 성공. `UnrealEditor`·`UnrealEditor-Cmd` 실행 0 확인 |
...
이 프로젝트는 2026-08-19 초기 감사 당시 `C:\project\Drone`에서 발견하고 정비했다. 아래의 "시작 시" 수치와 `91498b7`은 당시 사실을 보존한 역사 기록이다. `C:\project\Drone`은 현재 기준보다 뒤처진 복제본이므로 사용하지 않는다. 이번 확인 PC의 기준 프로젝트는 `C:\URproject\drone`이고 `main=origin/main=c3e6d38`이다.
...
현재 공유 main에는 Prototype Pawn/GameMode, 입력, Telemetry, Flight HUD, TUT-01 Course, TUT-02 Ordered Ring Gate, TUT-03 Segment/Lap 원본과 TUT-04B 이전 평균·Best·Delta 결과가 있다. NPC Faction·Rifle/Shotgun Profile, Smart Object Activity Tag, Character·Controller·Spawn Point·Station·예약 Component와 Drone Sight 대상 등록 기반도 `c3e6d38`에 병합됐다. 그 위 작업 Branch에서 Definition 6종과 대응 Station Blueprint 6종을 실제 생성했고 각 Slot·Activity·Definition·MG Mesh 연결을 검증했다. Game/Editor Build, 전체 `Drone.` 18/18, Blueprint 0/0/0을 통과했다. StateTree, 순찰, 아군 이동, Rifle/Shotgun 발사, MG 점유·공격은 아직 구현된 것으로 판정하지 않는다.
...
- `TUT-04` 수동 확인은 유지한다. `AI-SO-01`은 완료했고 다음 구현 우선순위는 `AI-NPC-01 → AI-PATROL-01 → AI-FRIEND-01`; 첫 물리 작업은 별도 `PHY-DF-00` Sandbox
